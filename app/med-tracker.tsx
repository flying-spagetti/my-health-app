  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [med, scheds] = await Promise.all([
        ...
        getDoseSchedulesByParent('medication', id),
      ]);
      ...
      if (selectedScheduleId || scheds.length > 0) {
        ...
        const [statsData, historyData] = await Promise.all([
          getTrackingStats('medication', id, scheduleId),
          getHistoryAggregates('medication', id, 30, scheduleId),
        ]);
        setStats(statsData);
    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: tokens.colors.background }}>
        <View style={styles.errorContainer}>
          <Text style={[styles.errorText, { color: tokens.colors.text }]}>Medication not found</Text>
        </View>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: tokens.colors.background }}>
      <ScrollView contentContainerStyle={[styles.content, { backgroundColor: tokens.colors.background }]}>
        <View style={styles.header}>
          <Text style={[styles.title, { color: tokens.colors.text }]}>{medication.name}</Text>
          <Text style={[styles.dosage, { color: tokens.colors.textMuted }]}>{medication.dosage}</Text>
        </View>

        {medication.prescribing_doctor && (
          <View style={[styles.infoCard, { backgroundColor: tokens.colors.elevatedSurface, borderColor: tokens.colors.border }]}>
            <Text style={[styles.infoLabel, { color: tokens.colors.textMuted }]}>Prescribed by</Text>
            <Text style={[styles.infoValue, { color: tokens.colors.text }]}>{medication.prescribing_doctor}</Text>
          </View>
        )}
        ...
        <View style={[styles.infoCard, { backgroundColor: tokens.colors.elevatedSurface, borderColor: tokens.colors.border }]}>
          ...
          <Text style={[styles.infoValue, { color: tokens.colors.text }]}>
            {formatDate(medication.start_date || medication.created_at)}
          </Text>
        </View>
        ...
        {medication.notes && (
          <View style={[styles.notesCard, { backgroundColor: tokens.colors.elevatedSurface, borderColor: tokens.colors.border }]}>
            ...
            <Text style={[styles.notesText, { color: tokens.colors.text }]}>{medication.notes}</Text>
          </View>
        )}
      </ScrollView>
    </SafeAreaView>
